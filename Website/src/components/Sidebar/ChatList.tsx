      ...
      {sessions.map((session) => (
        ...
        >
          <div className="flex items-center space-x-3 truncate">
            ...
            <span className="text-teal-100 truncate text-sm font-medium">{session.title}</span>
          </div>
          <button
            onClick={(e) => {
              e.stopPropagation();
            ...
            className="opacity-0 group-hover:opacity-100 text-blue-300 hover:text-red-400 transition-opacity duration-200"
          >
            <Trash2 size={18} />
          </button>
        </div>
